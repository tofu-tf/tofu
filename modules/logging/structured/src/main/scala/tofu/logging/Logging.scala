...
import tofu.{Delay, WithContext, higherKind}
...
trait Logging[F[_]] {
  ...
  def asService[Svc]: ServiceLogging[F, Svc] =
    (level: Level, message: String, values: LoggedValue*) => self.write(level, message, values : _*)
}

...
trait ContextEmbed[U[f[_]]] {
  final implicit def contextEmbed[F[_]: FlatMap](implicit FH: F WithContext U[F], UE: Embed[U]): U[F] =
    UE.embed(FH.context)
}
...
trait EmbeddedInstances[F[_]] {
  final implicit def contextEmbed[U[_[_]]](implicit FH: F WithContext U[F], UE: Embed[U], F: FlatMap[F]): U[F] =
    UE.embed(FH.context)
}

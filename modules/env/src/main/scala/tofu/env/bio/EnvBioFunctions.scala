...
private[bio] trait EnvBioFunctions extends EnvBioProducts { self: EnvBio.type =>
  ...
  /** Returns `EnvBio` that on execution is resulting in specified domain error being returned as `Left` Example:
    * {{{
  ...
  /** Returns current context, embedded in `EnvBio`. Example:
    * {{{
  ...
  /** Creates `EnvBio` from total, non-failing `Task`. Any `Throwable` raised by Task will result in fatal error being
    * thrown. Example:
    * {{{
    *   import monix.eval.Task
    *   import monix.execution.Scheduler.Implicits.global
  ...
  /** Creates `EnvBio` from `Task`. Any `Throwable` raised by Task will result in failed `EnvBio`, fixed to Left side.
    */
  def fromTask[A](task: Task[A]): EnvBio[Any, Throwable, A] =
    _ => task.onErrorHandleWith(e => Task.raiseError(UserError(e)))

  /** Creates `EnvBio` from total, non-throwing effect, resulting in a value of type `A`. Any `Throwable` raised by `x`
    * will result in fatal error being thrown.
    */
  def delayTotal[A](x: => A): EnvBio[Any, Nothing, A] =
    fromTaskTotal(Task.delay(x))

  /** Creates `EnvBio` from total effect. Any `Throwable` raised by `x` will result in failed `EnvBio` fixed to Left
    * side.
    */
  def delay[A](x: => A): EnvBio[Any, Throwable, A] =
    fromTask(Task.delay(x))

  /** Introduces an asynchronous boundary, effectfully shifting execution to another thread or call stack. An
    * implementaion delegates to Monix's implementation (see [[monix.eval.Task.shift]])
    */
  def shift: EnvBio[Any, Nothing, Unit] = fromTaskTotal(Task.shift)

  /** Introduces an asynchronous boundary, effectfully shifting execution to another thread or call stack. This
    * implementation differs in that underlying Monix `Task` will be executed on injected `Scheduler`. An implementaion
    * delegates to Monix's implementation (see [[monix.eval.Task.shift]])
    */
  def shift(ec: ExecutionContext): EnvBio[Any, Nothing, Unit] = fromTaskTotal(Task.shift(ec))

  /** Creates a new `EnvBio` that will sleep for given duration, and then continue execution. This operation doesn't
    * block a thread, the blocking is semantic.
    *
    * @param duration
    *   duration for 'sleeping', after which a tick will be emitted and execution will go on
    */

...
object ReadWrite {
   ...
   */
  final case class State[F[_], A](q: Queue[Op[F, A]],
                                  currentReaders: Int,
                                  writeLocked: Boolean,
  ...
  final class ConcurrentReadWrite[F[_], A](state: Ref[F, State[F, A]], maxReaders: Int)
    ...
    override def write: F[(A, A => F[Unit])] =
      Deferred[F, (A, A => F[Unit])].bracketCase { d =>
        state.modify {
          case s @ State(q, 0, false, _, _, a) if q.isEmpty => s.copy(writeLocked = true) -> d.complete(a -> releaseWrite)
    ...
    private def dropPrecedingTrashed(state: State[F, A]): State[F, A] =
      state.q match {
        case Reader(d) +: tl if state.trashedReaders(d) => dropPrecedingTrashed(state.copy(q = tl, trashedReaders = state.trashedReaders - d))
        case Writer(d) +: tl if state.trashedWriters(d) => dropPrecedingTrashed(state.copy(q = tl, trashedWriters = state.trashedWriters - d))
        case _ => state
      }

    private def extractReadersBatch(state: State[F, A], quantity: Int): (State[F, A], List[Reader[F, A]], Int) = {
      def impl(state: State[F, A], batch: List[Reader[F, A]], remain: Int, size: Int): (State[F, A], List[Reader[F, A]], Int) = {
        if (remain <= 0) (state, batch, size)
        else dropPrecedingTrashed(state) match {
          case s if s.q.isEmpty || s.q.head.isWriter => (state, batch, size)
          case s @ State((r @ Reader(_)) +: tl, _, _, _, _, _) => impl(s.copy(q = tl), r :: batch, remain - 1, size + 1)
        }
      }
      ...
      state.modify {
        dropPrecedingTrashed(_) match {
          ...
          case s =>
            ...
            cleanState.copy(a = newA, writeLocked = false, currentReaders = batchSize) -> readers.foldRight(F.unit)(_ *> _)
        }
      }.flatten

...
object Agent {
  ...
  sealed trait Watch[F[_], A] {
    val action: (A, A) => F[Unit]
  }
  final case class DisposableWatch[F[_], A](action: (A, A) => F[Unit]) extends Watch[F, A]
  final case class ConstantWatch[F[_], A](action: (A, A) => F[Unit])   extends Watch[F, A]
...
trait MakeAgent[F[_]] {
  ...
  def watchedAgent[K, A](a: A, n: Int = 1000): F[WatchedAgent[F, A]]
}
...
object MakeAgent {
  ...
  implicit def syncInstance[F[_]: Concurrent: Deferreds: Start]: MakeAgent[F] = new MakeAgent[F] {
    def agent[A](a: A, n: Int = 1000): F[Agent[F, A]] =
    ...
    def watchedAgent[K, A](a: A, n: Int = 1000): F[WatchedAgent[F, A]] =
      for {
        ...
        watchesRef <- newRef[F].of(List.empty[Agent.Watch[F, A]])
        _ <- (for {
              ...
              _ <- watches.traverse(w => w.action(oldA, newA).start)

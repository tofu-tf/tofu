  ...
  "ReadWrite" should {
    ...
    "acquire read lock when maxReaders is not reached. And then release" in {
      ...
      io.unsafeToFuture.map { case (initial, blocked, release, xs) =>
        ...
        blocked shouldBe State(Queue(), 4, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 1)
        release shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 1)
    ...
    "block on read lock" when {
      ...
      "maxReaders is exceeded. Wait and then release" in {
        ...
        io.unsafeToFuture.map { case (initial, blocked1, blocked2, blocked3, release, xs) =>
          ...
          blocked3 shouldBe State(Queue(), 2, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 1)
          release  shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 1)
          xs shouldBe List(1, 1, 1, 1)
        }
      }
      ...
      "someone is writing. Wait and then release" in {
        ...
        io.unsafeToFuture.map { case (initial, blocked, release1, release2, xs) =>
          ...
          release1 shouldBe State(Queue(), 1, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 2)
          release2 shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 2)
      ...
      "writer in queue. Wait and then release" in {
        ...
        io.unsafeToFuture.map { case (initial, blocked, writer, reader, result, xs) =>
          ...
          reader shouldBe State(Queue(), 1, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 2)
          result shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 2)
    ...
    "acquire write lock when there is no readers or writers. And then release" in {
      ...
      io.unsafeToFuture.map { case (initial, blocked, release) =>
        initial shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 1)
        blocked shouldBe State(Queue(), 0, writeLocked = true,  Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 1)
        release shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 2)
      }
    }
    ...
    "block on write lock" when {
      ...
      "someone already writing. Wait and then release" in {
        ...
        io.unsafeToFuture.map { case (initial, blocked, release1, release2, xs) =>
          ...
          release1 shouldBe State(Queue(), 0, writeLocked = true, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]],  2)
          release2 shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 3)
          xs shouldBe List(1, 2)
        }
      }
      ...
      "someone still reading. Wait and then release" in {
        ...
        io.unsafeToFuture.map { case (initial, blocked, release1, release2, xs) =>
          ...
          release1 shouldBe State(Queue(), 0, writeLocked = true, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]],  1)
          release2 shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 2)
    ...
    "remove tasks from q" when {
      ...
      "cancel on read happened" in {
        ...
        io.unsafeToFuture.map { case (initialState, blockedState, cancelState, releaseState1, releaseState2) =>
          initialState shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 1)
          ...
          releaseState2 shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 1)
        }
      }
    ...
    "tryRead" when {
      ...
      "read is available" in {
        ...
        io.unsafeToFuture().map { case (initial, blocked, release, i) =>
          initial shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 1)
          blocked shouldBe State(Queue(), 1, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 1)
          release shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 1)
          i shouldBe 1
        }
      }
      ...
      "read is unavailable" in {
        ...
        io.unsafeToFuture().map { case (initialState, afterState, non) =>
          initialState shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 1)
          afterState shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 1)
          non shouldBe None
        }
      }
    ...
    "tryWrite" when {
      ...
      "write is unavailable" in {
        ...
        io.unsafeToFuture().map { case (initialState, afterState, non) =>
          initialState shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 1)
          afterState shouldBe State(Queue(), 1, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 1)
          non shouldBe None
        }
      }
    ...
    "read, read, write, write, read, read, write, write" in {
      ...
      io.unsafeToFuture().map { case (initial, blocked, clean, is) =>
        ...
        clean shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 5)
    ...
    "write, write, read, read, write, write, read ,read" in {
      ...
      io.unsafeToFuture().map { case (initial, blocked, clean, is) =>
        initial shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 10)
        inside(blocked) { case State(q, 0, true, _, _, 10) =>
          q.map(_.isReader) shouldBe List(false, true, true, false, false, true, true)
        }
        clean shouldBe State(Queue(), 0, writeLocked = false, Set.empty[Deferred[IO, (Int, IO[Unit])]], Set.empty[Deferred[IO, (Int, Int => IO[Unit])]], 14)

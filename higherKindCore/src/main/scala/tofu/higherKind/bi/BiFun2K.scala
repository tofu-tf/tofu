...
trait BiFun2K[F[_, _], G[_, _], H[_, _]] {
  def apply[A, B](fa: F[A, B], ga: G[A, B]): H[A, B]
}

object BiFun2K {

  implicit class Ops[F[+_, +_], G[+_, +_], H[+_, +_]](private val f: BiFun2K[F, G, H]) extends AnyVal {
  ...
  class Applied[F[_, _], G[_, _]](private val __ : Boolean = true) extends AnyVal {
    type A1
    type B1
    def apply[H[_, _]](maker: Maker[F, G, H, A1, B1]): BiFun2K[F, G, H] = maker
  }

  abstract class Maker[F[_, _], G[_, _], H[_, _], A1, B1] extends BiFun2K[F, G, H] {

...
object ConfigItem {
  ...
  sealed trait SimpleValue[A] extends Value[Nothing, λ[`f[+_]` => SimpleC[f, A]]] {
    def valueType: ValueTypeSimple[A]
    def mapK[G[+_]: Functor](fk: Nothing ~> G): ConfigItem[G] = this
  }
  sealed trait IndexedValue[F[+_], I] extends Value[F, λ[`f[+_]` => IndexedC[f, I]]] {
    self =>
  ...
  sealed trait ValueTypeSimple[A]  extends ValueType[λ[`f[+_]` => SimpleC[f, A]]]
  sealed trait ValueTypeIndexed[I] extends ValueType[λ[`f[+_]` => IndexedC[f, I]]]
  sealed trait ValueTypeStream     extends ValueType[λ[`f[+_]` => StreamC[f]]]
  ...
  private type Upd[F[+_], I] = (F[ConfigItem[F]], I) => F[ConfigItem[F]]
  ...
  implicit class SyncConfigItemOps[F[+_]](val item: ConfigItem[F]) extends AnyVal {
    def tryParseSync[A: Configurable](implicit F: Sync[F], PR: ParallelReader[F]): F[(MessageList, Option[A])] =
      ConfigMonad.tryParseSync[F, A](item)
  ...
  implicit class IdConfigItemOps(val item: ConfigItem[λ[`+x` => x]]) extends AnyVal {

...
object Raise
    extends RaiseInstances
    with DataEffectComp[Raise]
    with ErrorsInstanceChain[Raise] {
...
sealed class RaiseInstances {
  final implicit def raiseUpcast[F[_], E, E1](
      implicit r: Raise[F, E],
      prism: Upcast[E, E1]): Raise[F, E1] =
...
object RestoreTo
    extends ErrorsToInstanceChain[λ[(f[_], g[_], e) => RestoreTo[f, g]]]
...
trait HandleTo[F[_], G[_], E] extends RestoreTo[F, G] {
  ...
  def attempt[A](fa: F[A])(implicit F: Functor[F],
                           G: Applicative[G]): G[Either[E, A]] =
...
trait Handle[F[_], E] extends HandleTo[F, F, E] with Restore[F] {
  ...
  def tryHandle[A](fa: F[A])(f: E => Option[A])(
  ...
  def recover[A](fa: F[A])(pf: PartialFunction[E, A])(
      implicit F: Applicative[F]): F[A] =
...
object Handle
    extends HandleInstances
    with DataEffectComp[Handle]
    with ErrorsInstanceChain[Handle] {
  ...
  trait ByRecover[F[_], E] extends Handle[F, E] {
    ...
    override def recoverWith[A](fa: F[A])(pf: PartialFunction[E, F[A]]): F[A] =
      recWith(fa)(pf)
    override def recover[A](fa: F[A])(pf: PartialFunction[E, A])(
        implicit F: Applicative[F]): F[A] =
...
object HasContextRun {
  def apply[F[_], G[_], C](
      implicit hcr: HasContextRun[F, G, C]): HasContextRun[F, G, C] = hcr
}

sealed class HandleInstances {
  final implicit def handleDowncast[F[_], E, E1](
      implicit h: Handle[F, E],
      prism: Downcast[E, E1]): Handle[F, E1] =
...
trait ErrorsToInstanceChain[TC[f[_], g[_], e] >: ErrorsTo[f, g, e]]
  ...
  final implicit def eitherTIntance[F[_], E](
      implicit F: Monad[F]): TC[EitherT[F, E, *], F, E] =
    new EitherTErrorsTo[F, E]
  final implicit def optionTIntance[F[_]](
      implicit F: Monad[F]): TC[OptionT[F, *], F, Unit] = new OptionTErrorsTo[F]
...
object Errors
    extends ErrorInstances
    with DataEffectComp[Errors]
    with ErrorsInstanceChain[Errors]

trait ErrorsInstanceChain[TC[f[_], e] >: Errors[f, e]] {
  final implicit def errorByCatsError[F[_]: ApplicativeError[*[_], E],
                                      E,
                                      E1: ClassTag: * <:< E]: TC[F, E1] =
    new HandleApErr[F, E, E1] with RaiseAppApErr[F, E, E1] with Errors[F, E1]
}

trait ErrorInstances {
  final implicit def errorPrismatic[F[_], E, E1](
      implicit e: Errors[F, E],
      prism: Subset[E, E1]): Errors[F, E1] =
    new FromPrism[F, E, E1, Errors, Subset] with RaisePrism[F, E, E1]
    with HandlePrism[F, E, E1] with Errors[F, E1]

  final implicit def readerTErrors[F[_], R, E](
      implicit F: Errors[F, E]): Errors[ReaderT[F, R, *], E] =
    new Errors[ReaderT[F, R, *], E] {
      def raise[A](err: E): ReaderT[F, R, A] =
        ReaderT.liftF(F.raise(err))
      def tryHandleWith[A](fa: ReaderT[F, R, A])(
          f: E => Option[ReaderT[F, R, A]]): ReaderT[F, R, A] =

...
Let's take a look at this tofu.example.  
...
* by asking that name from user:
  For tofu.example here we create named instance and pass it explicitly.
...
instance should be in scope. In the tofu.example they are generated by macro, we just need to import it:
...
logged.  
Let's take a look at the tofu.example:

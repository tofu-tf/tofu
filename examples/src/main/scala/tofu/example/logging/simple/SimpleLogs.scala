package tofu.example.logging.simple
...
object MovingCompany {
  ...
  def make[F[_]: Monad: Logging.Make]: MovingCompany[F] = new MovingCompany[F] {
    implicit val logging: Logging[F] = Logging.Make[F].forService[CargoService[F]]
...
object Warehouse {
  def makeSmall[F[_]: Monad: Logging.Make]: Warehouse[F] = new Warehouse[F] {
    implicit val logging: Logging[F] = Logging.Make[F].forService[CargoService[F]]
